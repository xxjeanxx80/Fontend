...
import { decodeAccessToken, getRoleHomePath } from '@/utils/auth';
    ...
    loginMutation.mutate(
      { email, password },
      {
        onSuccess: (response) => {
          ...
          const decoded = response.data.access_token ? decodeAccessToken(response.data.access_token) : null;
          const next = getRoleHomePath(decoded?.role ?? response.data.user?.role ?? null);
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl shadow-slate-200/60 dark:bg-navy-900 dark:shadow-none">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
            Access your personalised dashboard as a customer, spa owner, or administrator with a single login.
          </p>
        </div>
